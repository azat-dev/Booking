...
public class CommandsSignUpApiController implements CommandsSignUpApi {

    private final CommandsSignUpApiDelegate delegate;

    public CommandsSignUpApiController(@Autowired(required = false) CommandsSignUpApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new CommandsSignUpApiDelegate() {});
    }

    @Override
    public CommandsSignUpApiDelegate getDelegate() {
        return delegate;
    }
