...
public final class MapListingDetailsUpdatedChange implements Serializer<ListingDetailsUpdated.Change, ListingDetailsFieldsDTO> {
    ...
    public ListingDetailsFieldsDTO serialize(ListingDetailsUpdated.Change change) {
        ...
        final var status = change.status().map(Enum::name).map(ListingStatusDTO::fromValue);

        final var description = change.description().map(v -> v.map(ListingDescription::getValue));

        final var propertyType = change.propertyType().map(v -> v.map(Enum::name).map(PropertyTypeDTO::valueOf));

        final var roomType = change.roomType().map(v -> v.map(Enum::name).map(RoomTypeDTO::valueOf));

        final var guestsCapacity = change.guestsCapacity().map(mapGuestCapacity::serialize);

        final var address = change.address().map(v -> v.map(mapAddress::serialize));

        return ListingDetailsFieldsDTO.builder().status(toNullable(status)).title(toNullable(title)).description(toOptionalNullable(description)).propertyType(toOptionalNullable(propertyType)).roomType(toOptionalNullable(roomType)).guestCapacity(toNullable(guestsCapacity)).address(toOptionalNullable(address)).build();
    }
    ...
    public ListingDetailsUpdated.Change deserialize(ListingDetailsFieldsDTO dto) {
        return new ListingDetailsUpdated.Change(OptionalField.from(dto.getStatus()).map(Enum::name).map(ListingStatus::valueOf), OptionalField.from(dto.getTitle()).map(ListingTitle::dangerouslyMakeFrom), OptionalField.fromNullable(dto.getDescription()).map(v -> v.map(ListingDescription::dangerouslyMakeFrom)), OptionalField.fromNullable(dto.getPropertyType(), v -> PropertyType.valueOf(v.name())), OptionalField.fromNullable(dto.getRoomType(), v -> RoomType.valueOf(v.name())), OptionalField.from(dto.getGuestCapacity()).map(mapGuestCapacity::deserialize), OptionalField.fromNullable(dto.getAddress(), mapAddress::deserialize));
    }
